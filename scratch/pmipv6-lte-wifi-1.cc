...
const std::string traceFilename = "pmipv6-lte-wifi-1";

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("Pmipv6LteWifi1");
 ...
 * network. The MAG functionality is installed on a separate node.
 *
 * Node 0 - SGW
 * Node 1 - PGW
 * Node 2 - Remote Host
...
main (int argc, char *argv[])
{
  ...
  Ptr<Ipv6> ipv6 = wifiMag->GetObject<Ipv6> ();
  ...
  Ipv6InterfaceContainer wifiMagApIpIfaces = ipv6h.Assign (wifiMagApDevs);
  ...
  if (enablePcap)
    {
      ...
      wifiPhy.EnablePcap (traceFilename, wifiApDev);
      csmaHelper.EnablePcap (traceFilename, wifiMagApDevs);
    }
