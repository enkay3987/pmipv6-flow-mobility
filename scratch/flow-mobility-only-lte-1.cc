...
const std::string traceFilename = "flow-mobility-only-lte-1";
 ...
 */
NS_LOG_COMPONENT_DEFINE ("FlowMobilityOnlyLte1");
...
void RxTrace (std::string context, Ptr<const Packet> packet, Ptr<Ipv6> ipv6, uint32_t interfaceId)
{
  ...
  Ipv6Header ipv6H;
  packet->PeekHeader (ipv6H);
  NS_LOG_DEBUG (context << " " << ipv6H << " " << interfaceId);
}
...
//  Config::Connect ("/NodeList/1/$ns3::Ipv6L3Protocol/Rx", MakeCallback (&RxTrace));
//  Config::Connect ("/NodeList/2/$ns3::Ipv6L3Protocol/Tx", MakeCallback (&TxTrace));
//  Config::Connect ("/NodeList/1/$ns3::Ipv6L3Protocol/Drop", MakeCallback (&DropTrace));
