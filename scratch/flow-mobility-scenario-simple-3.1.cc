...
const std::string traceFilename = "flow-mobility-scenario-simple-3.1";

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("FlowMobilityScenarioSimple3.1");
...
void CreateFlowInterfaceList (Ptr<FlowInterfaceList> flowInterfaceList)
{
  ...
  entry = new FlowInterfaceList::Entry (flowInterfaceList);
  entry->SetPriority (17);
  ts.SetProtocol (6);
  ts.SetDestinationPort (1000);
  entry->SetTrafficSelector (ts);
  interfaceIds.clear ();
  interfaceIds.push_back (8);
  interfaceIds.push_back (4);
...
{
  NS_LOG_UNCOND ("Installing UDP Applications");
  ...
  for (int i = 0; i < n; i++, dlPort += 1000)
    {
      ...
      switch (i)
      {
         ...
         case 2:   // non-conversational video
           {
             dlClient.SetAttribute ("Interval", TimeValue (MilliSeconds(20)));
             dlClient.SetAttribute ("MaxPackets", UintegerValue (0xffffffff));
         ...
         case 3:
          {
            ...
            dlClient.SetAttribute ("MaxPackets", UintegerValue (0xffffffff));
            dlClient.SetAttribute ("PacketSize", UintegerValue (args.packetSize));
            break;
          }
         }
...
void InstallTcpApplication(Args args)
{
  NS_LOG_UNCOND ("Installing TCP Applications");
  uint16_t dlPort = 1000;
  BulkSendHelper source ("ns3::TcpSocketFactory",
  Inet6SocketAddress (args.ueIpIface.GetAddress (0, 1), dlPort));
  //source.SetAttribute("MaxBytes",UintegerValue(2200000));
  // Set the amount of data to send in bytes. Zero is unlimited.
  ApplicationContainer sourceApps = source.Install (args.remoteHost);
  sourceApps.Start (Seconds (0.0));
  sourceApps.Stop (Seconds(140));
  PacketSinkHelper sink ("ns3::TcpSocketFactory",
  Inet6SocketAddress (Ipv6Address::GetAny (), dlPort));
  ApplicationContainer sinkApps = sink.Install (args.ueNode);
  ...
  for (uint32_t i = 0; i < wifiFlowIds.size (); i++)
    {
      ...
      std::list<uint32_t> interList=entry->GetInterfaceIds();
      std::list<uint32_t>::iterator iter;
      iter=interList.begin();
      if(*iter==4)
        return;
      else
        {
          NS_LOG_INFO ("Switching flows onto Wi-Fi");
          interfaceIds.push_back (4);
          interfaceIds.push_back (8);
          entry->SetInterfaceIds (interfaceIds);
        }
  ...
  for (uint32_t i = 0; i < wifiFlowIds.size (); i++)
    {
      ...
      if (*iter == 8)
        ...
        {
          NS_LOG_INFO ("Switching flows back to LTE");
          interfaceIds.push_back (8);
          interfaceIds.push_back (4);
          entry->SetInterfaceIds (interfaceIds);
        }
  ...
  for (FlowMapI flow = flowStatsMap.begin(); flow != flowStatsMap.end(); flow++)
    {
      ...
      oss << "FlowID=" << flow->first << ", protocol=";
      switch(t.protocol)
      {
      case(6):
        oss << "TCP, ";
      ...
      oss << "sourceIp=" << t.sourceAddress << ", sourcePort=" << t.sourcePort << ", destinationIp=" << t.destinationAddress << ", destinationPort=" << t.destinationPort
        ...
        << ", throughput(kbps)=" << (((double)flow->second.rxBytes * 8) / ((flow->second.timeLastRxPacket.GetSeconds ()-flow->second.timeFirstRxPacket.GetSeconds() ) * 1000))
        << ", meanDelay(ms)=" << (flow->second.rxPackets != 0 ? (flow->second.delaySum.GetMilliSeconds() / flow->second.rxPackets) : 0)
        << ", meanJitter(ms)=" << (flow->second.rxPackets != 0 ? (flow->second.jitterSum.GetMilliSeconds() / (flow->second.rxPackets)) : 0)
        << ", dataRate(kbps)=" << (((double)flow->second.txBytes * 8) / ((flow->second.timeLastTxPacket.GetSeconds () - flow->second.timeFirstTxPacket.GetSeconds()) * 1000));
...
void ComputeFlowStats (FlowMonitorHelper *flowMonHelper, FlowMap &flowStatsMap)
{
  ...
  PrintFlowStats (flowMonHelper,flowStatsMap);
}

void GetFlowStats(Ptr<FlowMonitor> flowMon, FlowMonitorHelper *flowMonH)
{
  FlowMap flowMap=flowMon->GetFlowStats();
  FlowMapI iter;
  Simulator::Schedule(Seconds(2),&ComputeFlowStats,flowMonH,flowMap);
  Simulator::Schedule(Seconds(2),&GetFlowStats,flowMon,flowMonH);
}
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("FlowMobilityScenarioSimple3.1", static_cast<LogLevel>(LOG_LEVEL_DEBUG | LOG_PREFIX_TIME));
  ...
  double simTime = 160;
  uint64_t interPacketInterval = 1000;
  double internetDelay = 100;
  ...
  cmd.AddValue ("packetSize", "The packet size for the flow which is moved. (in bytes)", packetSize);
  cmd.AddValue ("velocity", "Velocity in m/s", v);
  cmd.AddValue ("internetDelay", "Internet delay (in ms)", internetDelay);
  ...
  p2ph.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (internetDelay)));
  ...
  Simulator::Schedule (Seconds (10), &InstallApplications, args, 4);

  NodeContainer flowMonNodes;
  flowMonNodes.Add (remoteHost);
  flowMonNodes.Add (ue);
  FlowMonitorHelper *flowMonHelper;
  flowMonHelper = new FlowMonitorHelper ();
  Ptr<FlowMonitor> flowMon = flowMonHelper->Install (flowMonNodes);
  Simulator::Schedule(Seconds(10),&GetFlowStats,flowMon,flowMonHelper);
