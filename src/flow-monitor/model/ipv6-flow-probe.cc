...
{
  NS_LOG_FUNCTION (this << ipHeader << ipPayload << interface);
  FlowId flowId;
...
{
  NS_LOG_FUNCTION (this << ipHeader << ipPayload << reason << ipv6 << ifIndex);
#if 0
  switch (reason)
    {
...
{
  NS_LOG_FUNCTION (this << ipPayload);
  // remove the tags that are added by Ipv6FlowProbe::SendOutgoingLogger ()
  Ipv6FlowProbeTag fTag;
