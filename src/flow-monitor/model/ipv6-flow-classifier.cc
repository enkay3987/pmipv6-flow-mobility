...
bool
Ipv6FlowClassifier::Classify (Ipv6Header &ipHeader, Ptr<const Packet> ipPayload,
                              uint32_t *out_flowId, uint32_t *out_packetId)
{
  ...
  *out_packetId = m_flowPktIdMap[*out_flowId];
  ipHeader.SetFlowLabel (*out_packetId);
  return true;
}
                              ...
                              uint32_t *out_flowId)
{
  ...
  // try to insert the tuple, but check if it already exists
  std::map<FiveTuple, FlowId>::iterator it = m_flowMap.find (tuple);

  // if the insertion succeeded, we need to assign this tuple a new flow identifier
  if (it != m_flowMap.end ())
    {
      *out_flowId = it->second;
      return true;
    }
  return false;
}
