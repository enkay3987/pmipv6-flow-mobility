...
const std::string traceFilename = "flow-mobility-scenario-simple-1";

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("FlowMobilityScenarioSimple1");
...
void RetrieveLatestWifiSnr(NodeContainer nodes)
{
  Simulator::Schedule(Seconds(0.1),&RetrieveLatestWifiSnr,nodes);
  Ptr<FlowConnectionManager> flowConnectionManager1=nodes.Get(0)->GetObject<FlowConnectionManager>();
  std::cout<<"\npresent snr value at time "<<Simulator::Now().GetSeconds()<<" is:";
  FlowConnectionManager::snrTime  snr=flowConnectionManager1->GetWifiLatestSnrTime();
  std::cout<<snr.time<<" "<<snr.snr;
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable("FlowMobilityScenarioSimple1", LOG_LEVEL_DEBUG);
  ...
  Ptr<WifiNetDevice> ueWifiNetDev=DynamicCast<WifiNetDevice>(ueWifiDev);
  ...
  flowConnectionManagerH.InstallWifi(ue,ueWifiNetDev,n);

  Ptr<FlowConnectionManager> flowConnectionManager=flowConnectionManagerH.GetFlowConnectionManager(ue);
 // LogComponentEnable ("FlowConnectionManager", LOG_LEVEL_ALL);
  NodeContainer node;
  node.Add(ue);
  Simulator::Schedule(Seconds(0.1),&RetrieveLatestWifiSnr,node);
